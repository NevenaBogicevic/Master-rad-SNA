...
snowball <- function(x) {
  sb <- names(table(x$replyToSN)[table(x$replyToSN) > mean(table(x$replyToSN), na.rm=TRUE)])
  sb <- union(sb, unique(x$screenName))
  return(sb)
}
...
count_replyTo <- function(tw, replyingTo) {
  cnt = 0
  for(i in 1:nrow(tw)) {
    if(!is.na(tw$replyToSN[i]) &
       tw$replyToSN[i] == replyingTo &
       !startsWith(tw$text[i], paste0("@", tw$replyToSN[i])))
      cnt = cnt + 1
  }
  return(cnt)
}

matrica_povezanosti_simpler <- function(x) {
  sp <- unique(x$screenName)
  lsp <- length(sp)
  mp <- matrix(data = 0, nrow = lsp, ncol = lsp)
  colnames(mp) <- sp
  row.names(mp) <- sp
  for (i in 1:lsp) {
    ...
    for (j in 1:lsp) {
      ...
      if (any(grepl(sp[j], l$text))) {
        mp[i, j] <- table(grepl(sp[j], l$text))["TRUE"]
      }
      if (sp[j] %in% l$replyToSN) {
        mp[i, j] <- mp[i, j] + count_replyTo(l, sp[j])
      }
    }
  }
  mp[is.na(mp)] <- 0
...
# Funkcija met_graf racuna globalne metrike izabrane mreze 
...
podgraf_hom_lis <- function(graf, lis = NULL) {
  ...
  ##JJ: dodata provera da bi se izbegla greska u slucaju kada nema konekcija u grafu
  if (ecount(subgraf) == 0) {
    print(paste("Skipping", lis, ". list element due to the lack of connections"))
    return(NULL)
  }
  ...
  asortativnost_str <- ifelse(is.nan(asortativnost), "NA", asortativnost)
  naslov <- str_c(unique(V(graf)$lista)[lis], ": r = ", asortativnost_str, "; tr = ", tranzitivnost)
...
podgraf_hom_fra <- function(graf, fra = NULL) {
  ...
  tranzitivnost <- round(transitivity(subgraf), 3)
  V(subgraf)$shape[which(betweenness(subgraf) == max(betweenness(subgraf)))] <- "square"
  naslov <- str_c(unique(V(graf)$frakcija)[fra], ": r = ", asortativnost, "; tr = ", tranzitivnost)
  subgraf_vis <- toVisNetworkData(subgraf)
  visNetwork(nodes = subgraf_vis$nodes, edges = subgraf_vis$edges, width = "100%", height ="100vh", main = naslov) %>%
    visIgraphLayout(layout = "layout_nicely")
}